...
Считается дисперсия по следующей формуле: \[\sigma^{2} = \frac{\sum_{i = 1}^{n}(x_i - \overline{x})^2}{n}\]
...
__Троттлингом__ называют занижение тактовой частоты процессора с целью не допустить термических повреждений при перегреве.

Особенно это можно наблюдать на ноутбуках, когдам система охлаждения не справляется со своей задачей при
