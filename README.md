...
$\sigma^{2} = \frac{\sum(x_i - x_{\text{ср}})^2}{n}$
...
Зная, что троттлинг на процессорах intel начинается при нагреве до 100-105 градусов Цельсия,
можем сказать, что 77 градусов - __хороший показатель__.
...
| Версия                                                        | Время выполнения __без__ -O3, сек  |Время выполнения __с__ -O3, сек|ускорение относительно базовой версии| ускорение относительно предыдущей версии|
|:--------------------------------------------------------------|----------------|---------------|--|--|
|(0) Базовая версия                                             | 8,15 +- 0,01   | 2,25 +- 0,01  |3,62|3,62|
|(1) Базовая версия + asm_CRC32Hash()                           | 1,10 +- 0,01   | 0,55 +- 0,01  |14,81|4,09|
|(2) Базовая версия + asm_CRC32Hash() + SIMD поиск              |       -        | 0,40 +- 0,01  |20,38|1,36|
|(3) Базовая версия + asm_inline_CRC32Hash()                      | 1,10 +- 0,01   | 0,57 +- 0,01  |14,30|3,94|
|(4) Базовая версия + asm_inline_CRC32Hash() + SIMD поиск         |       -        | 0,39 +- 0,01  |20,89|1,46|
...
Судя по данным в таблице, можем видеть, что __только asm_inline_CRC32Hash()__ ускоряет программу в __7,40__ раз.
В то время как __только -O3__ ускоряет в __3,59__ раза.
